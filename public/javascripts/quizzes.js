...
define([
  ...
  quiz = {
    ...
    updateFormAnswer: function($answer, data, ignoreCurrent) {
      ...
      if (answer.answer_misconception == "") {
        $answer.find(".answer_misconception option[value='1']").attr('selected','selected');
      } else {
        $answer.find(".answer_misconception option[value=" + answer.answer_misconception + "]").attr('selected','selected');
      }

      data.answer_misconception = $answer.find(".answer_misconception option:selected").val();
    ...
    updateFormQuestion: function($form) {
      ...
      result.textValues = ['answer_weight', 'answer_misconception', 'answer_text', 'answer_comment', 'blank_id', 'id', 'match_id'];
  ...
  function makeDisplayAnswer(data, escaped) {
    ...
    data.answer_misconception = data.misconception || data.answer_misconception
    ...
    $list.each(function(i) {
      ...
      if (question.question_type != 'calculated_question') {
        $question.find(".answer").each(function() {
          ...
          $answer.find(".answer_misconception").text($answer.find(".answer_misconception").val())
          
          var answerData = $answer.getTemplateData({
            textValues: ['answer_exact', 'answer_error_margin', 'answer_range_start', 'answer_range_end', 'answer_weight', 'answer_misconception', 'numerical_answer_type', 'blank_id', 'id', 'match_id', 'answer_text', 'answer_match_left', 'answer_match_right', 'answer_comment'],
  ...
  function generateFormQuiz(quiz) {
    ...
    for(var idx in quiz.questions) {
      ...
      for(var jdx in question.answers) {
          ...
          data[jd + '[answer_misconception]'] = answer.answer_misconception;
  ...
  $(document).ready(function() {
    ...
    $("#question_form_template").submit(function(event) {
      ...
      var $answers = $question.find(".answer").each(function(i) {
        ...
        data.answer_misconception = $answer.find("select option:selected").val()
      ...
      $displayQuestion.find(".answers").each(function() {
        $(this).find(".answer").each(function(index){
          $(this).find(".answer_misconception").text(question.answers[index].answer_misconception);
        });
      });
